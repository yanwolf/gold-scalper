...
`app/static/dashboard.html` 現在多了「纏論分析」面板，顯示：
- 分型數 / 筆數 / 合併K棒數
- 最新中樞的上下界(ZG/ZD)
- 背馳訊號橫幅(紅色高亮=有背馳，灰色=無)
- 最近的筆列表(方向、價格區間、漲跌)

可以切換1分鐘/5分鐘/15分鐘K線週期，方便在資料量還不夠5分鐘判斷時，先用1分鐘週期驗證邏輯有沒有正常運作。

## 資料持久化：加 PostgreSQL 避免重啟遺失歷史資料

之前的版本tick/trade歷史只存在記憶體，Zeabur重新部署或容器重啟就會清空，
分析模組(尤其纏論的5分鐘K線)每次都要重新累積資料才能用，很不方便。

現在加了 `app/db.py`，把 Binance 逐筆成交批次寫進 PostgreSQL，服務啟動時會自動
從資料庫回填最近的歷史資料到記憶體，不用每次重啟都從零開始。

### 設定步驟

1. 在 Zeabur 專案裡另外新增一個服務：Marketplace 搜尋 **PostgreSQL**，一鍵部署
2. 回到這個 gold-scalper 服務的環境變數設定頁，新增一個變數：
   ```
   DATABASE_URL=${POSTGRES_CONNECTION_STRING}
   ```
   Zeabur 會自動把 PostgreSQL 服務的連線字串注入進來(這是Zeabur的變數引用語法)，
   不用手動複製帳號密碼
3. 重新部署後，打 `/health` 確認 `database_persistence_enabled` 是 `true`

### 運作方式

- 每20秒把新累積的逐筆成交批次寫進資料庫一次(不是每筆都馬上寫，減少資料庫負擔)
- 服務啟動時自動從資料庫撈最近的歷史成交回填進記憶體(最多回填到 `MAX_TRADE_HISTORY` 上限，目前是20000筆)
- **沒設定 `DATABASE_URL` 也完全沒問題**：程式會自動偵測、退回純記憶體模式運作，跟之前的行為一樣，不會讓服務起不來
- 寫入/讀取資料庫失敗只會記錄log，不會讓即時資料流中斷
