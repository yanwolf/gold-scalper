...
EXEMPT = {
    ...
    "LiveAckResponse.test_close_fill_price_from_trades_when_response_lacks_it": ("對照組", "Lesson54.test_r54_exit_does_not_close_other_position",
        "平倉送單前的確認(r54)在突變下查不到→照樣送單(原本的設計)；這項測的是送單之後的事。送單前的確認本身由被引用的那一項驗證(突變下失敗)"),
    "RealSqlDb.test_r76_exit_backfill_is_rescheduled_after_restart": ("對照組", "Lesson54.test_r54_exit_does_not_close_other_position",
        "平倉送單前的確認(r54)在突變下查不到→照樣送單(原本的設計)；這項測的是送單之後的補登記號(r76)。送單前的確認本身由被引用的那一項驗證(突變下失敗)"),
    "RealSqlDb.test_r76_marker_cleared_when_backfill_gives_up": ("對照組", "Lesson54.test_r54_exit_does_not_close_other_position",
        "平倉送單前的確認(r54)在突變下查不到→照樣送單(原本的設計)；這項測的是送單之後的補登記號(r76)。送單前的確認本身由被引用的那一項驗證(突變下失敗)"),
    "RealSqlDb.test_r76_marker_is_written_before_the_close_record": ("對照組", "Lesson54.test_r54_exit_does_not_close_other_position",
        "平倉送單前的確認(r54)在突變下查不到→照樣送單(原本的設計)；這項測的是送單之後的補登記號(r76)。送單前的確認本身由被引用的那一項驗證(突變下失敗)"),
